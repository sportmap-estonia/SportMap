...
namespace SportMap.PL.Controllers
{
        ...
        ILogger<ProfileController> logger) : ControllerBase
    {
            ...
            Guid id, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (result.HasError)
                {
                    ...
                    logger.LogError("{controller}.{method}: {error}", nameof(ProfileController), nameof(GetById), result.ErrorMessage);
            ...
            {
                logger.LogError(e, "{controller}.{method}: Unhandled exception", nameof(ProfileController), nameof(GetById));
                return TypedResults.InternalServerError();
            }
        }
            ...
            string username, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (result.HasError)
                {
                    ...
                    logger.LogError("{controller}.{method}: {error}", nameof(ProfileController), nameof(GetByUsername), result.ErrorMessage);
                    return TypedResults.InternalServerError();
                }
            ...
            {
                logger.LogError(e, "{controller}.{method}: Unhandled exception", nameof(ProfileController), nameof(GetByUsername));
                return TypedResults.InternalServerError();
            }
        }
            ...
            Guid id, CancellationToken cancellationToken)
        {
            ...
            {
                logger.LogError(e, "{controller}.{method}: Unhandled exception", nameof(ProfileController), nameof(GetPosts));
                return TypedResults.InternalServerError();
            }
        }
    }
